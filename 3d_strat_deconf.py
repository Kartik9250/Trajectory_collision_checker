...
# 1. Calculate the total distance of the user's path first
total_user_dist = calculate_path_length(waypoints)
total_duration = user_end_time - user_start_time

# 2. Keep track of time as we move through segments
current_time_acc = user_start_time

for i in range(len(waypoints) - 1):
    p1, p2 = np.array(waypoints[i]), np.array(waypoints[i+1])
    seg_dist = np.linalg.norm(p2 - p1)
    
    # Calculate how much of the total time budget this segment gets
    if total_user_dist > 0:
        seg_duration = (seg_dist / total_user_dist) * total_duration
    else:
        seg_duration = total_duration # Handling single-point edge case
    
    # Calculate points needed for 0.01s resolution
    n_points = max(2, int(seg_duration / TIME_RESOLUTION))
    ...
    for j, point in enumerate(seg):
        ...
        # Calculate time linearly based on progress through this specific segment
        fraction = j / (n_points - 1) if n_points > 1 else 0
        point_time = current_time_acc + (fraction * seg_duration)
        
        full_path.append(point)
        path_times.append(point_time)
    
    # Advance the clock for the next segment
    current_time_acc += seg_duration
...
print(f"\n !!Collisions detected: {len(collision_zones)} zone(s)!!")
if len(collision_zones) == 0:
    ...
    for i, (entry_idx, exit_idx, entry_pos, exit_pos, entry_time, exit_time) in enumerate(collision_zones):
        # Format coordinates for the string
        en_coord = f"({entry_pos[0]:.1f}, {entry_pos[1]:.1f}, {entry_pos[2]:.1f})"
        ex_coord = f"({exit_pos[0]:.1f}, {exit_pos[1]:.1f}, {exit_pos[2]:.1f})"
        
        print(f"  Zone {i+1}: Entry at t={entry_time:.2f}s xyz coord={en_coord}, "
              f"Exit at t={exit_time:.2f}s xyz coord={ex_coord}, "
...
if SHOW_VISUALIZATION:
    ...
    print("\nVisualization disabled by user variable.")